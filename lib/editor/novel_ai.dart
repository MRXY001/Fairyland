class NovelAI {
  String autoPuncWhitelists = '吗哼呀呢么嘛啦呵吧啊哦哪呸滚';
  String shuoBlacklists =
  ...
  String daoWhiteLists = "，说口嘴的着回答地释述笑哭叫喊吼脸骂";
  String wenBlackLists = "一请想去若试不莫问想要你我敢来";
  String symbolPairLefts = "([{（［｛‘“<〈《〔【「『︵︷︹︻︽︿﹁";
  String symbolPairRights = ")]}）］｝’”>〉》〕】」』︶︸︺︼︾﹀﹂";
  String sentPuncs = "。？！；~—…?!";
  String sentSplitPuncs = "，。？！；~—…?!;:\"“”,.";
  String sentSplits = "\n\t，。？！；~—…?!;:\"“”,.";
  String quoteNoColons =
  ...
  bool isClimaxPattern = false; // 是否是高潮模式

  /// 获取标点
  /// @param para 整个段落
  /// @param pos  光标在段落的相对位置
  String getPuncInPara(String para, int pos) {
    String left1 = pos > 0 ? para.substring(pos - 1, pos) : '';
    String left2 = pos > 1 ? para.substring(pos - 2, pos - 1) : '';
    String left3 = pos > 2 ? para.substring(pos - 3, pos - 2) : '';
    String right1 = pos < para.length ? para.substring(pos, pos + 1) : '';

    // 获取短句
    int left = pos, right = pos;
    while (left > 0 && !isSentSplit(para.substring(left - 1, left))) {
      left--;
    }
    while (
        right < para.length && !isSentSplit(para.substring(right, right + 1))) {
      right++;
    }
    String sent = para.substring(left, right);

    // 获取长句
    left = pos;
    right = pos;
    while (left > 0 && !isSentPuncOrQuote(para.substring(left - 1, left))) {
      left--;
    }
    while (right < para.length &&
        !isSentPuncOrQuote(para.substring(right, right + 1))) {
      right++;
    }
    String sent2 = para.substring(left, right);

    // 判断引号
    bool inQuote = false;
    left = para.lastIndexOf("“", pos - 1);
    right = para.lastIndexOf("”", pos - 1);
    String desc = '';
    if (left > right) {
      // 在引号里面
      inQuote = true;
  ...
  /// 获取句子语气
  int getDescTone(String sent) {
    int tone = -1;

    if (isClimaxPattern)
      return tone = 1;
    else if (sent.contains("轻"))
      tone = 0;
  ...
  String getTalkTone(String sent, String sent2, int tone, String left1,
    ...
    if (sent.contains("是不")) {
      ...
      punc = "，";
    else if (sent.contains("真") &&
        !sent.contains("真理") &&
        !sent.contains("真假") &&
        !sent.contains("真事") &&
        !canRegExp(sent, "真.{0,3}不是")) {
      if (sent.contains("真的") || sent.contains("真是")) {
        if (sent.contains("真的是") || sent.contains("真是")) {
          if (sent.contains("啊"))
            ...
            punc = "！";
          else if (sent.contains("了")) if (sent.contains("太") ||
              sent.contains("好"))
            punc = "！";
          else
            punc = "？";
    ...
    } else if (sent.contains("太") &&
        "了啦吧".indexOf(sent.substring(sent.length - 1)) >
            -1) // 带“太”的都是厉害的，除了部分名词，应该都是感叹号
      punc = "！";
      ...
      if (isKnowFormat(sent) == true)
        ...
        punc = "？";
    } else if (sent.contains("可能是") &&
        canRegExp(sent, ".*[特别|真的|格外|非常].*") == true)
      punc = "！";
      ...
      if (isKnowFormat(sent) == true &&
        ...
        sent.substring(sent.length - 1) == "不" &&
        !sent.contains("不不"))
      punc = "？";
    else if (sent.contains("还不") ||
        canRegExp(sent, "^(你他她)们?(怎么|为什么|干嘛|为何)还(没|不)")) {
      if (sent.contains("给我"))
        punc = "！";
      else if (tone == 1)
        punc = "！";
      ...
      punc = "！";
        ...
        !sent.contains("呢") &&
        !sent.contains("了")) {
      if (sent.contains("你都要") &&
          sent.contains("这") &&
          sent.indexOf("这") > sent.indexOf("你都要"))
        punc = "？";
      else
        punc = "！";
    } else if (sent == "我要")
      ...
      ;
        ...
        sent.length > 6 &&
        sent.substring(sent.length - 1) == "了") {
      if (tone == 1)
        punc = "！";
      else if (tone == 0)
        punc = "。";
      ...
      if (sent == "要不")
        ...
        punc = "？";
    } else if (sent.contains("可不") &&
        (sent.contains("可不可") || sent.endsWith("可不")))
      punc = "？";
      ...
      if (tone == 2)
        ...
        punc = "！";
    } else if (canRegExp(sent, ".{1,3}(无论|不管)(怎么|什么|任意|任何)") &&
        !sent.contains("真的")) {
      ...
      if (sent.contains("什么")) {
        ...
        if (tone == 1)
          ...
          ;
        else if (sent.contains("没有什么") ||
            sent.contains("没什么") ||
            sent.contains("无什么") ||
            canRegExp("不.{1,3}什么", sent))
          ;
          ...
          if (sent.contains("怎么")) if (sent.contains("不"))
            ;
          else
            punc = "？";
          ...
          ;
        else if (sent.contains("说了什么") &&
            sent.indexOf("的", sent.indexOf("说了什么") + 1) > -1)
          ;
        ...
        ;
      else if (sent.contains("多么")) if (sent.contains("啊") ||
          sent.contains("呀") ||
          sent.contains("呢") ||
          sent.contains("诶"))
        punc = "！";
      ...
      if (isKnowFormat(sent) == true &&
          sent.contains("我") &&
          !sent.contains("怎么") &&
          sent.indexOf("知") < sent.indexOf("怎样") &&
          sent.indexOf("明") < sent.indexOf("怎样") &&
          sent.indexOf("懂", 0) < sent.indexOf("怎样"))
        ;
      else
        punc = "？";
      ...
      if (sent.contains("奈何"))
        ...
        if (sent.contains("无论") ||
            sent.contains("不管") ||
            isKnowFormat(sent) == true) if (tone == 1)
          punc = "！";
        else {}
        else
          punc = "？";
      ...
      punc = "？";
        ...
        (tone == 2 ||
            ...
            sent.contains("几时") ||
            sent.contains("样了") ||
            (sent.startsWith("几") && sent.endsWith("了")) ||
            (sent.contains("你") && sent.indexOf("你") < sent.indexOf("几"))))
      punc = "？";
      ...
      if (tone == 0)
        ...
        sent.length >= 3 &&
        sent.substring(sent.length - 3).indexOf("居然") > -1)
      punc = "……";
    else if (sent.contains("居然")) {
      if (tone == 2)
        punc = "？";
      ...
      ;
    else if (sent.contains("也不能") &&
        sent.indexOf("也不能") <= 2 &&
        "吗呢".indexOf(sent.substring(sent.length - 1)) == -1)
      ;
      ...
      if (tone == 0 ||
          ...
          sent.contains("一些") ||
          sent.contains("还没") ||
          sent.indexOf("不是") > sent.indexOf("听说") ||
          sent == "听说" ||
          sent.indexOf("前") > sent.indexOf("听说") ||
          sent.contains("时") ||
          sent.contains("听说过") ||
          sent.contains("却") ||
          (sent.contains("没听说") &&
              "吧吗啊".indexOf(sent.substring(sent.length - 1)) > -1) ||
          sent.contains("都听说") ||
          sent.indexOf("她") > sent.indexOf("听说") ||
          sent.indexOf("他") > sent.indexOf("听说") ||
          sent.indexOf("它") > sent.indexOf("听说") ||
          sent.endsWith("听说") ||
          sent.contains("这"))
        ;
      else if (tone == 1)
      ...
      if (isKnowFormat(sent) == true && sent.substring(sent.length - 2) == "知道")
        ...
        ;
      else if (sent.length > 7 &&
          sent.substring(0, 3) == "不知道" &&
          sent.substring(sent.length - 1) == "的")
        punc = "？";
      ...
      punc = "？";
    else if (sent.contains("定") &&
        canRegExp(sent, "[必定|一定|肯定|定要|定把|定将|定会|定能|定可|定是|定非]") == true &&
        !sent.contains("不一定") &&
        !sent.contains("不") &&
        !sent.contains("确定") &&
        !sent.contains("稳定") &&
      ...
      if (sent.contains("你确定"))
        ...
        punc = "！";
    } else if (sent.contains("滚") &&
        !sent.contains("打滚") &&
        !sent.contains("翻滚") &&
        !sent.contains("滚动") &&
        !sent.contains("靠滚"))
      punc = "！";
    else if (sent.contains("混账") ||
        sent.contains("混蛋") ||
        sent.contains("可恶") ||
        sent.contains("变态") ||
        sent.contains("难以置信")) {
      ...
      if (tone == 0)
        ...
        punc = "？";
    } else if (sent.contains("有点") &&
        "啊吗呢吧呀么".indexOf(sent.substring(sent.length - 1)) == -1)
      ;
      ...
      if (tone == 1)
        ...
        punc = "？";
      else if (sent.contains("会") &&
          sent.contains("我") &&
          sent.indexOf("我") < sent.indexOf("会"))
        punc = "？";
      ...
      if (sent.contains("还是"))
        ...
        punc = "，";
      else if ((sent.indexOf("啊") < sent.length - 1 &&
              sent.substring(sent.length - 2, sent.length - 1) != "啊") ||
          tone == 0)
        ;
      else if (tone == 1)
        punc = "！";
  ...
  bool canRegExp(String str, String pat) {
  ...
  /// 是否是双引号
  bool isQuoteSymbol(String str) {
    return str == "“" || str == "”";
  }

  /// 是否是句子标点或双引号
  bool isSentPuncOrQuote(String str) {
    return isSentPunc(str) || isQuoteSymbol(str);
  }
